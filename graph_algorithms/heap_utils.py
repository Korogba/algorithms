...
def get_min(heap_items) -> Optional[object]:
    if len(heap_items) == 0:
        return None
...
def peek_min(heap_items) -> Optional[object]:
    if len(heap_items) == 0:
        return None
    return heap_items[0]
...
def bubble_down(heap_items, parent_index=0):
    ...
    while left_child_index < len(heap_items):
        if right_child_index < len(heap_items) and heap_items[right_child_index] < heap_items[left_child_index]\
                and heap_items[right_child_index] < heap_items[parent_index]:
        ...
        elif heap_items[left_child_index] < heap_items[parent_index]:
...
def bubble_up(heap_items, node_index=-1):
    ...
    while heap_items[node_index] < heap_items[parent_index]:
...
def remove_arbitrary_item(heap_items, index) -> object:
    ...
    if heap_items[index] < heap_items[parent_index]:
